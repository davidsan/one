...
public class DTNSimGUI extends DTNSimUI {
	...
	public void update(boolean forcedUpdate) {
		...
		if (!forcedUpdate
				&& guiUpdateInterval > (SimClock.getTime() - this.lastUpdate)) {
			return;
		}
	...
	private class PlayfieldMouseHandler extends MouseAdapter implements
		...
		public void mouseClicked(MouseEvent e) {
			...
			Coord offset = MapBasedMovement.getOffset();
			System.out
					.println((field.getWorldPosition(new Coord(p.x, p.y))
							.getX() + offset.getX())
							+ ";"
							+ (-1 * (field
									.getWorldPosition(new Coord(p.x, p.y))
									.getY() + offset.getY())));
		}
